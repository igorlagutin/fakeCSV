...
class GeneratorViewTest(TestCase):
    ...
    def test_unauthenticated_dataset_view(self):
        target_url = reverse('schema_dataset', kwargs={"pk": self.schema_by_user_1.pk})
        response = self.client.get(target_url)
        self.assertRedirects(
            response,
            '/accounts/login/?next={}'.format(target_url),
    ...
    def test_authenticated_dataset_by_wrong_user(self):
        target_url = reverse('schema_dataset', kwargs={"pk": self.schema_by_user_1.pk})
        self.client.login(username=self.USERNAME2, password=self.PASS)
        response = self.client.get(target_url)
        self.assertEqual(response.status_code, 403)
